...
class GraphSampler(torch.utils.data.Dataset):
    ...
    '''
    def __init__(self, G_list, features='default', normalize=True, attention_feat='default', max_num_nodes=0):
        self.adj_all = []
        self.len_all = []
        self.feature_all = []
        self.label_all = []
        
        self.attention_feat_all = []
        ...
        for G in G_list:
            ...
            if features == 'default':
                # print('using default feature')
                f = np.zeros((self.max_num_nodes, self.feat_dim), dtype=float)
                for i,u in enumerate(G.nodes()):
                    # print(i,u)
                    f[i,:] = util.node_dict(G)[u]['feat']
                self.feature_all.append(f)
                # print(f) # f = max_num_nodes x feat_dim
            elif features == 'id':
            ...
            if attention_feat == 'id':
                self.attention_feat_all.append(
                        np.hstack((np.identity(self.max_num_nodes), self.feature_all[-1])) )
            else:
                self.attention_feat_all.append(self.feature_all[-1])
        
        self.feat_dim = self.feature_all[0].shape[1]
        self.attention_feat_dim = self.attention_feat_all[0].shape[1]
    ...
    def __getitem__(self, idx):
        ...
        return {'adj':adj_padded,
                ...
                'num_nodes': num_nodes,
                'attention_feats':self.attention_feat_all[idx].copy()}
